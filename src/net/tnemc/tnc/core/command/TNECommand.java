...
public abstract class TNECommand {
  ...
  protected TheNewChat plugin;

  public TNECommand(TheNewChat plugin) {
    this.plugin = plugin;
  }
  ...
  public boolean execute(CommandSender sender, String command, String[] arguments) {
    ...
    if(sub == null && !arguments[0].equalsIgnoreCase("help") && !arguments[0].equalsIgnoreCase("?")) {
      sender.sendMessage(ChatColor.RED + "Unable to locate the specified command.");
      return false;
    }
    ...
    if(!sub.canExecute(sender)) {
      sender.sendMessage(ChatColor.RED + "Unable to use the specified command.");
      return false;
    }
